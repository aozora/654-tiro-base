import { db, players, tournaments, matches, playersOnMatches } from './index.js';
import type { Player, Tournament, Match, PlayerOnMatch } from './schema.js';
...
export async function getPlayerStats(tournamentId: string, playerId: string): Promise<PlayerStats> {
	...
	const distinctMatchIds = await db.selectDistinct({
		matchId: playersOnMatches.matchId
	})
	.from(playersOnMatches)
	.innerJoin(matches, eq(playersOnMatches.matchId, matches.id))
	...
	if (distinctMatchIds.length > 0) {
		...
		const allMatchResults = await db.select()
			...
			.innerJoin(matches, eq(playersOnMatches.matchId, matches.id))
			.where(and(
				inArray(playersOnMatches.matchId, distinctMatchIds.map(m => m.matchId)),
				eq(matches.tournamentId, tournamentId)
			));

		// Calculate wins
		for (const distinctMatch of distinctMatchIds) {
			const matchResults = allMatchResults.filter(x => x.PlayersOnMatches.matchId === distinctMatch.matchId);
			const maxPoints = Math.max(...matchResults.map(x => x.PlayersOnMatches.points));
			const winner = matchResults.find(x => x.PlayersOnMatches.points === maxPoints);
...
export async function updatePlayerPicture(id: string, picture: string): Promise<Player> {
	const result = await db.update(players)
		.set({ picture })
		.where(eq(players.id, id))
		.returning();
...
// Tournaments
export async function getTournaments(): Promise<Array<Tournament & { matches: Match[] }>> {
	const tournamentsData = await db.select()
		.from(tournaments)
...
export async function deleteMatchDeep(id: string): Promise<Match> {
	...
	await db.delete(playersOnMatches)
		.where(eq(playersOnMatches.matchId, id));
...
export async function upsertMatchPlayer(matchId: string, playerId: string, points: number): Promise<PlayerOnMatch> {
	// Try to update first
	const updateResult = await db.update(playersOnMatches)
		.set({ points })
		.where(and(
			eq(playersOnMatches.playerId, playerId),
	...
	const insertResult = await db.insert(playersOnMatches)
...
export async function removePlayerFromMatch(matchId: string, playerId: string): Promise<PlayerOnMatch> {
	const result = await db.delete(playersOnMatches)
		.where(and(
			eq(playersOnMatches.playerId, playerId),
			eq(playersOnMatches.matchId, matchId)
		))
...
export async function getLeaderboard(tournamentId: string) {
	...
	return db.select({
		playerId: playersOnMatches.playerId,
		totalPoints: sql<number>`sum(${playersOnMatches.points})::int`
	})
	.from(playersOnMatches)
	.where(inArray(playersOnMatches.matchId, tournamentMatches.map(m => m.id)))
